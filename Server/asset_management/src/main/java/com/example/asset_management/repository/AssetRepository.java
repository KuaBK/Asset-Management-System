...
public interface AssetRepository extends JpaRepository<Asset, Long> {
    ...
    @Query(
            "SELECT COUNT(a) FROM Asset a WHERE a.building.id = :buildingId AND a.room.id = :roomId AND a.assetType = :assetType")
    Long countAssetsByBuildingAndRoomAndType(
            @Param("buildingId") Long buildingId,
            @Param("roomId") Long roomId,
            @Param("assetType") AssetType assetType);

    @Query(
            "SELECT COUNT(a) FROM Asset a WHERE a.building.id = :buildingId AND a.room.id = :roomId AND a.assetType = :assetType AND a.isBroken = true")
    Long countBrokenAssetsByBuildingAndRoomAndType(
            @Param("buildingId") Long buildingId,
            @Param("roomId") Long roomId,
            @Param("assetType") AssetType assetType);
